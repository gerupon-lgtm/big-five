...
test("P-0 through P-6 are all approved", async () => {
  const model = await loadPresentationReviewModel({ sourceDir: SOURCE_DIR });
...
test("approval metadata consistency is required by both model and renderer", async () => {
  ...
  draftWithMetadata[6].status = "draft";
  assert.throws(
...
test("P-0 through P-6 approvals render truthfully", async () => {
  const model = await loadPresentationReviewModel({ sourceDir: SOURCE_DIR });
  const report = renderPresentationReview(model);
  assert.match(report, /承認状況: approved=P-0, P-1, P-2, P-3, P-4, P-5, P-6/);
  assert.doesNotMatch(report, /; draft=/);
  ...
  assert.match(report, /## P-6 ペア称号 31〜40（approved）/);
});
