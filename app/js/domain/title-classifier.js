...
function compareSalience(left, right) {
  const salienceDifference = right.salience - left.salience;
  if (Math.abs(salienceDifference) > FLOAT_COMPARISON_EPSILON) return salienceDifference;
  ...
  const varianceDifference = left.variance - right.variance;
  if (Math.abs(varianceDifference) > FLOAT_COMPARISON_EPSILON) return varianceDifference;
...
function boundaryFlags(factorResults, rankedSalientFactors, threshold, questionCount) {
  const flags = factorResults.flatMap(({ factorId, rawMean }) => [2.5, 3.5]
    .filter((boundary) => Math.abs(rawMean - boundary) <= threshold + FLOAT_COMPARISON_EPSILON)
    .map((boundary) => Object.freeze({ type: "factor-near-band-boundary", factorId, boundary, threshold, questionCount })));
  if (rankedSalientFactors.length >= 3 && rankedSalientFactors[1].salience - rankedSalientFactors[2].salience <= threshold + FLOAT_COMPARISON_EPSILON) {
