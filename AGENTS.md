...
- 通常版とベータ版の機能フラグをテストし、通常版の診断フローで自動外部送信0件を維持する。F-023だけは50問詳細結果で利用者が明示操作した時のシゴトソケット遷移を許可する。
...
- 公開結果URL、アカウント、通常公開版の実行時DB・分析送信をMVPへ追加しない。ベータ匿名集計はF-017、明示的なシゴトソケット遷移はF-023と`docs/api-design.md`の範囲だけを例外とする。
