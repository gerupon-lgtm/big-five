...
- 正式版の基盤: `app/`
- 現在の版: `mvp-1.3.0`
- 実装済み: 20問簡易プレビュー、50問詳細結果、途中保存・再開、履歴・比較・削除、ココロパレット／ココロアロマ、共有カード・PNG保存、50問履歴からのシゴトソケット連携
- 通常版の自動外部送信: 0件（CSP `connect-src 'none'`）。F-023だけは利用者操作でシゴトソケットへ遷移
- 既存UIプロトタイプ: `prototype-big-five/`（正式版とは分離）

開始画面には展開式のツール紹介、閉じたシゴトソケット連携説明、現在版を表示します。全削除では結果履歴と途中回答に加え、同じ画面セッション内の再開状態も破棄するため、削除後に再開ボタンは表示されません。
