...
- 正式版の基盤: `app/`
- 現在の版: `mvp-0.2.0`
- 完了タスク: T-001〜T-004、T-032（基盤、固定尺度、採点・51分類、回答状態、シゴトソケット連携）
- 実装済みの主要画面: 回答、20問簡易プレビュー、50問詳細結果、履歴、比較
- 継続中: T-005、T-006、T-008Aの残ゲートとT-007〜T-012。詳細は`docs/tasks.md`を参照
- 既存UIプロトタイプ: `prototype-big-five/`（正式版とは分離）

正式版は固定50問の診断フローと結果保存・再表示まで動作します。未承認コンテンツ、正式キャラクター／演出資産、共有カード、公開・受入作業は引き続きリリースゲートとして管理しています。
...
- 外部連携・ベータ匿名集計API: `docs/api-design.md`
