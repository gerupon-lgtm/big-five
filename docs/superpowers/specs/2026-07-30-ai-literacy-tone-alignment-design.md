...
回答画面は他画面と同じ通常の共通ヘッダーを使う。履歴・比較の右側操作、既存の戻り先とクリック処理は変更しない。
...
- 回答画面の`中断してトップへ`、履歴の`トップ画面へ`、比較の`履歴へ戻る`を維持する。回答画面だけにsticky変種を設けない。
...
現在の説明ブロックを`このツールについて`として白いメインフィールド内へ入れ、二重カードにはしない。

- 外側の白いメインフィールドを主パネルとする。
- `このツールについて`は主パネル内の小見出しと説明ブロックにする。
- 追加の大きな角丸枠と大きな影は外す。
- 診断開始・再開ボタンは説明ブロックの直後へ置き、12px間隔の同幅レスポンシブgridにする。
...
同日の追補設計・実装は`docs/superpowers/specs/2026-07-30-mobile-header-start-actions-followup-design.md`を正典とする。回答・20問分岐だけに付いていた`is-sticky`を外して通常の共通ヘッダーへ統一し、ローカル実ブラウザの320／360／414pxで第一候補の`中断してトップへ`が1行のままブランドと重ならず、横overflowもないことを確認した。ブランドとの実測間隔は約`5.4px`／`30.7px`／`12px`だった。開始画面は`このツールについて`へ改め、開始・再開ボタンを12px間隔の同幅gridへ変更した。320pxと360pxは同幅1列、414pxは各`164px`の同幅2列となり、3幅とも横overflowはなかった。
