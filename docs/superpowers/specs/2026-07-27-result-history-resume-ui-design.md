...
- 履歴ヘッダーに`…`管理メニューを置き、実施日時・20問／50問で識別できる個別削除、全削除、版情報をまとめる。個別削除を通常カードへ常時表示しない。
- 全削除は確認を必須とする。
...
- ボトムシートを利用できない環境: 同一DOM内のインライン展開へ切り替える。
