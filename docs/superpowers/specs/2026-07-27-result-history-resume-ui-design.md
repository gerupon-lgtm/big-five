...
- 文字を過度に縮小して1行へ押し込まない。
- 設問本文はスマートフォンで20px相当、広い画面で22px相当、太字、行間1.5を基準とする。CSSでは`1.25rem`から`1.375rem`の範囲とし、トップ画面用の大見出し規則を継承させない。
- 5件法の選択肢は16px相当、行間1.5、最低高56px、上下14px・左右16px相当を基準とする。設問本文との間は24px相当、選択肢間は12px相当とする。
- 現在位置と全体数は14〜16px相当とし、現在位置だけを文字の太さで強調できる。設問本文より強くしない。
- この文字スケールは回答画面固有とし、結果・履歴画面へ一律適用しない。
- 参考画面からは文字階層と余白だけを参照し、配色、ブランド意匠、画面構成は移植しない。
...
`振り返りのヒント`は測定結果と分離する。20問では簡易結果をもとにした参考情報である旨だけを注記し、50問では共通補助文を設けない。各提案を`〜してみませんか。`で結んで任意性を示す。各称号は固定順3件を持ち、20問は1件目だけ、50問は1件目を初期表示し、`ほかのヒントを見る`で残り2件を同一階層に展開する。第三階層、ランダム選択、共有物への転載は行わない。
...
- 履歴画面の`履歴削除`から、`履歴の管理`モーダルを開く。実施日時・20問／50問で識別できる個別削除、全削除、`診断時の情報を見る`をまとめ、個別削除を通常カードへ常時表示しない。
- 管理画面には明示的な`閉じる`を置く。背景部分のタップ、Escキーでも閉じられるようにし、内部操作のタップでは閉じない。
- 開いたときは管理画面内へフォーカスを移し、閉じた後は起動元の`履歴削除`へ戻す。背景をキーボード・読み上げ操作の対象に残さない。
- 個別削除と全削除は、ブラウザ標準confirmではなく同じモーダル内のアプリ固有確認を必須とする。確認を取り消した場合は管理一覧へ戻し、起動した削除操作へフォーカスを戻す。
