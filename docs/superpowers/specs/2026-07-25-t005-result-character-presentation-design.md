...
- 状態: 設計承認済み。Q-006ドメイン、永続化、live／保存済みS-003/S-004、完答controller、Q-012全51体・loaderを実装済み。人手Content Approval／Q-013統合待ち
- 対象版: `mvp-0.1.0`
...
2026-07-27に承認された結果の情報優先度、因子の段階展開、履歴、中断再開のUI詳細は`docs/superpowers/specs/2026-07-27-result-history-resume-ui-design.md`を優先する。本書のQ-006/Q-012/Q-013データ契約、フォールバック、承認ゲートは引き続き有効である。
...
Q-006の称号ラフに加え、版付き根拠、全因子・全称号の文面、決定的合成、診断時文章を保持するsnapshotは実装・独立レビュー済みである。文面は`result-text-v1 initial reviewed copy`であり、根拠台帳のE-1／E-2／E-3／F-1／F-2／F-3は2026-07-28にapproved、E-4〜E-5は`draft`、T-0〜T-4／F-4〜F-5／X-1〜X-2は`reviewed`だが人手approval recordなしのため`Content Approval pending`である。実装レビューを人手内容承認へ読み替えず、Q-006を完全解決またはT-005全体完了とは扱わない。
...
- E-4〜E-5: 根拠台帳どおり`draft`、承認日なし、`Content Approval pending`。
...
- T-0〜T-4、F-4〜F-5: `reviewed`。implementation auditと独立レビューは完了したが、人手approval recordはないため`approved`と記録しない。
...
1. Q-006: E-1／E-2／E-3／F-1／F-2／F-3は2026-07-28にapproved。E-4〜E-5、T-0〜T-4、F-4〜F-5、X-1〜X-2の各gateに必要な人手approval recordを根拠台帳へ記録する。残gateの承認までは`result-text-v1 initial reviewed copy`かつ`Content Approval pending`として扱う。
