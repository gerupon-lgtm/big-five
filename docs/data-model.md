...
- カード画像は中央ブランド、称号ラベルと称号、植物だけのちょうど6本の枝を左右から伸ばして上辺・下辺を閉じない開いたアーチ、透過猫、固定順5因子、`ココロアロマ`の見出しと副題、20問／50問のモード、アプリ版、注意書き、透過素材画付き代表3件を持つ。v2は円形ガイド、白色・ニュートラル色の円形面、矩形プレートを置かず、顔・胴体・しっぽを隠さない。ブランドの画面用`iconPath`とカード用高解像度`cardIconPath`を分離し、共有モデルは後者を描画へ渡す。詳細な称号理由と内部版IDは画像へ表示しない。
