...
- 診断データとしてブラウザへ永続化するのは途中回答と結果スナップショットだけとする。T-038の最終連携resultIdは診断内容を含まない独立状態として例外的に1件だけ保持する。
- 生回答は途中回答にだけ保持し、完答時に削除する。結果履歴・共有物へ移さない。
- ProgressRecordは診断種別ごとに直近1件だけを保持する。20問簡易プレビュー表示後も追加30問へ進める間は保持し、`簡易プレビューで終了する`または50問完答後の明示確定で削除する。
...
- 50回答が揃った後に完答確認から戻った場合、`currentIndex`は0〜49の見直し位置を保持できる。見直し中かどうかと「回答を完了する」の表示は保存schemaへ追加せず画面内状態で管理し、再読込時は50回答から完答確認を復元する。
- T-037の太字回答数、プログレスバー、`前の質問`表記は`ProgressRecord.currentIndex`の既存値を表示するだけで、保存フィールド・schemaVersion・結果データを変更しない。
...
`questionCount`が確定した判定入力では、`itemCount = questionCount / 5`、`keyedSum = round(rawMean * itemCount)`を導出し、`rawMean === keyedSum / itemCount`を必須とする。`varianceNumerator = round(variance * itemCount * itemCount)`についても、`variance === varianceNumerator / (itemCount * itemCount)`を必須とする。さらに1〜5の回答値countを列挙し、`keyedSum`、`varianceNumerator`、`directionalSupportCount`が同一回答集合から同時に到達可能であることを検証する。`questionCount`不明時は4項目・10項目の全適合候補を検証し、いずれかで全統計が到達可能なら受理する。`FactorResult`の公開フィールドは変更しない。
...
### 3.10 SigotosocketLinkState

最後に受け渡し操作をした履歴カードを識別するため、正式な`StorageEnvelope`とは別のlocalStorageキーへexact 2フィールドの`{ schemaVersion: 1, resultId }`を1件だけ保存する。`resultId`はUUID v1〜v5だけを受け付け、未知フィールド、連携コード、5因子、生回答、受信成否、連携先の保存・削除状態を持たない。別結果の受け渡し時は全体を上書きする。欠落・不正・読込失敗は未連携表示として扱い、書込失敗は外部遷移を妨げない。全削除成功時はこの独立状態も削除し、個別削除では残存してもカードIDと一致しないため表示されない。
