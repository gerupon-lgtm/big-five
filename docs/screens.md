...
- 共有テキストには香りの素材例を含めない
- 選択配色と猫が同系色の場合も確認できる、縁取り・影を含む完成時と同一のプレビュー。猫の背後へ白色・ニュートラル色の円形面は置かない
...
- 猫は1024×1024透過WebP、香りの素材画は短辺800px以上の透過PNG、ブランドは512px PNGを使い、枠・リース・棒・文字はCanvasへ直接描画する。完成カードをSVGからラスタライズしない。
