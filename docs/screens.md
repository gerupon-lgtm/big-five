...
香調候補は「ひと息つきたい」「気持ちを切り替えたい」「静かに取り組みたい」の固定順で、各2件、合計6件を持つ。`ココロアロマ`は閉じた状態でも3場面の代表素材画像を各1件見せるティーザーとし、外枠、淡い面、右向き矢印で全体を押せるカードとして示す。1回開くと矢印を下向きに回転して6候補と共通注記を一括表示する。展開後の場面見出しは先頭候補との関係が自然になる上余白を持つ。場面ごとの内側アコーディオンは設けない。アロマは5因子と同じ相互排他グループに入り、パレットは開閉対象にしない。開閉状態は色・香り・診断結果の保存値へ影響させない。
