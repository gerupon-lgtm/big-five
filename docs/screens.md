...
- 称号の代表2因子と全5因子
- 名前付きレーダーと、固定因子順の因子名、CSP下でも実スコアを反映する棒、数値、矢印と「詳しく見る」を含む全幅の選択行
- 初期状態では因子と本文を閉じる。同時に1因子だけを展開し、「今の傾向」「活かしやすい強み」「強みの裏返り」「仕事」「人間関係」「ストレス」「振り返りと行動ヒント」のカテゴリ名を表示する
- カテゴリ名自体を開く二段階目で診断時の本文を表示する。同時に1カテゴリだけを開き、別因子を開いたら既存のカテゴリも閉じる。汎用サマリや三段目の「詳しく見る」は置かない
...
診断直後の`トップへ戻る`は保存済みResultSnapshotなら確認なしで`#/start`へ移動する。履歴保存に失敗したlive結果では、戻ると再表示できないことを確認し、取消時は同じ結果画面を維持する。20問簡易プレビューの最下部には`結果履歴を見る`を表示しない。50問結果の`結果履歴を見る`は文字リンクではなくボタン表示とする。履歴から開いた保存済み結果では、ヘッダーと最下部に`履歴一覧に戻る`を表示し、最下部はボタン表示とする。この状態では`トップへ戻る`と`もう一度診断する`を表示しない。履歴から開いた20問結果に互換な途中回答がある場合、操作領域は`あと30問続ける`と`履歴一覧に戻る`の2ボタンだけとし、`中断してトップへ`と`簡易プレビューで終了する`を表示しない。診断直後のlive結果は従来の操作を維持する。
...
- 初期状態では色候補を閉じ、見出し、副題、`3つの色合いから、結果カードの雰囲気を選べます。`の1行説明だけを表示する。
- 展開後の標準1件と代替2件は縦1列に並べ、標準／代替ラベルと色名の不自然な途中分割を避ける。`診断結果は変わりません`は表示しない。
- 展開後に`選んだ色は、画像として共有・保存する結果カードに反映されます。`を表示する。色候補を選択した後の再描画ではパレットを開いたままにし、別の外側パネルを開いた時は閉じる。
...
香調候補は「ひと息つきたい」「気持ちを切り替えたい」「静かに取り組みたい」の固定順で、各2件、合計6件を持つ。通常結果では`ココロアロマ`と副題`～あなたらしさから着想した香り～`までを初期表示し、候補全体は閉じて開始する。外側を展開すると閉じた3場面を表示し、場面タップで当該2候補の素材例1〜2件、短い印象、説明を展開する。共有プレビューでは各場面の代表1件、合計3件へ要約し、素材例と短い印象、共通注記`香りをイメージするための素材例です`を表示する。共有テキストには素材例を含めない。

ココロパレットとココロアロマは同じアクセシブルな開閉規則を使う。同時に開く外側パネルは1件だけとし、片方を開くともう片方を閉じる。ココロアロマ内で同時に開く場面も1件だけとし、別場面を開くと直前の場面を閉じる。開閉状態は色・香り・診断結果の保存値へ影響させない。

2026-07-31にS-003/S-004へ実装した。色候補は名称・3色見本・標準／代替1／代替2・選択状態を併記し、選択時はResultSnapshotの`selectedPaletteId`だけを更新する。保存失敗時も画面内の選択と結果全体を維持して通知する。色選択による再描画では外側パレットの展開状態を維持する。ココロアロマは3場面を縦に並べ、各場面の2候補へ香調名、素材例1〜2件、説明を表示する。現在の心理状態や効果を示さず、使用方法を案内しない共通注記を添える。
