...
結果文の表示入力は`RenderedResultText` 7件（`titleSubtitle`、`titleReason`、`FACTOR_ORDER`順の5観察文）である。20問簡易プレビューの限界と「50問で変わり得る」注意は、7件の版付き結果文とは別の必須UI文面として表示する。
...
猫画像またはCanvasが失敗しても、称号、結果文、根拠への導線、共有テキストへの経路を同じ結果画面から利用できるようにする。
