...
共有CTAは通常フロー内にブランドアイコン付きの`今回の称号カードを見てみませんか`／`称号カードを見る`の1箇所だけ置き、追従表示しない。文言は`app/js/config/ui-copy.js`へ集約する。5状態の表示・操作は次のとおりである。
...
T-007で`renderShareScreen`を実装し、`#/share?resultId=<UUID>`から対象ResultSnapshotを検証して共有モデル、PNG、共有テキストを生成する。画面状態は`data-share-view`の`card`、`details`、`zoom`で管理し、同じPNG Blobから作った同じ画像URLを再生成せずに使う。ルートを離れる時はObject URLを解放する。現行`card-template-v2`の補正版透過ラスタリースは2026-08-02にユーザー承認済みである。
