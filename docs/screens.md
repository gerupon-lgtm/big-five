...
- 全画面に`Big Five｜自己理解支援ツール`を小さく控えめな共通ヘッダーとして表示する。狭幅では意味の境界だけで改行し、結果の称号・猫や設問本文より強くしない。
...
結果文の表示入力は`RenderedResultText` 7件（`titleSubtitle`、`titleReason`、`FACTOR_ORDER`順の5因子それぞれの観察文）である。20問簡易プレビューの限界と「50問で変わり得る」注意は、7件の版付き結果文とは別の必須UI文面として表示する。
...
- 履歴ヘッダーの`…`管理メニューから、日時・20問／50問で識別できる個別削除、全削除、版情報
- 個別削除と全削除は確認後に実行する
