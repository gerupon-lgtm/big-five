...
T-007で`renderShareScreen`を実装し、`#/share?resultId=<UUID>`から対象ResultSnapshotを検証して共有モデル、PNG、共有テキストを生成する。画面状態は`data-share-view`の`card`、`details`、`zoom`で管理し、同じPNG Blobから作った同じ画像URLを再生成せずに使う。ルートを離れる時はObject URLを解放する。現行`card-template-v2`は後者の補正版透過ラスタリースを維持するが、上下中央付近の膨らみを抑える形状調整は保留する。
...
- 現行リースは猫の背面合成と下端追従を維持する。上下中央付近の膨らみを抑える形状修正は保留し、本UI追補では素材とCanvas配置を変更しない。
