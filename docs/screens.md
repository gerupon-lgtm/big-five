...
T-007で`renderShareScreen`を実装し、`#/share?resultId=<UUID>`から対象ResultSnapshotを検証して共有モデル、PNG、共有テキストを生成する。画面状態は`data-share-view`の`card`、`details`、`zoom`で管理し、同じPNG Blobから作った同じ画像URLを再生成せずに使う。ルートを離れる時はObject URLを解放する。実ブラウザ視覚承認は自動テストと別に記録する。
...
- 共有テキスト全文
- 20問／50問の区別
- 猫、称号、固定順5因子の棒、短文、色候補反映、`ココロアロマ`の代表3件（場面名、素材例、短い印象）、共通注記、注意書き、版
- 共有テキストには香りの素材例を含めない
...
2026-07-31の実ブラウザ検証では、1080×1800のPNG読込、1280×720と320×480の初期`card`、`details`内スクロール、`zoom`の縦横スクロール、3状態での同一画像URL、操作領域、ブラウザログ0件を確認した。検証中に見つかったBlob画像のCSP遮断、320×480の操作重なり、称号理由の左右はみ出しは、CSP、短高さレイアウト、3行以内の折り返し描画と回帰テストで解消した。
