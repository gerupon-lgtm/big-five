...
結果画面は称号・猫hero、称号理由、称号別`titleReflection`、名前付きレーダー、固定順5因子の行・実スコアを反映する棒・数値、因子の一回開閉、設問構成sheet、4つの固定方法sheetを表示する。因子を開くと、保存済みの全カテゴリ本文を1枚のカードへまとめて表示する。5因子とココロアロマは同時に1件だけ開け、別の因子またはアロマを開くと先の領域を閉じる。パレットはこの排他グループに入れない。5因子スコア領域へ`拡大して見る`は置かない（S-005の共有画像を拡大して見る操作は維持する）。
...
- 履歴管理の通常操作は他画面と同じ緑系の副ボタン、削除候補は淡い赤系、確認後の確定削除だけを明確な赤系とする。`診断時の情報を見る`の展開本文は小さな補助文字とし、版名と値を読み分けられる二列表示にする
