...
- アプリ内URLへ回答・スコア・結果・個人情報を含めない。F-023の連携先URLフラグメントだけは5因子数値を固定形式で含める。
- 直接アクセス時は端末保存と現在状態を検証し、安全な画面へ遷移する。
- アプリ内の画面遷移後は縦位置を先頭へ戻し、遷移前画面のスクロール位置を引き継がない。
...
`result-text-v1`は237件の不変な履歴互換版で、別管理のQ-006全18 approval gateは2026-07-28にすべてapprovedとなっている。現行ES Modules runtimeの`result-text-v2`は基本237件に、TR-0〜TR-4承認済みの称号別`titleReflection`153件を加えた390件で、基本文面には承認済み修正27件を含む。v2の結果文と根拠の対応行は267件、実行時の根拠定義は引き続き固定6件である。Q-013はP-0〜P-6を承認済みで、全51称号の標準1＋代替2パレット、3場面×各2香調、共有代表が確定し、`presentation-v2` ES Modules runtime、S-003/S-004結果DOM、T-007の共有モデル・Canvas・S-005へ接続済みである。2026-07-31に追加された履歴preview、比較表示、`ココロパレット`／`ココロアロマ`開閉の追補はT-008Bとして実装済みである。正式共有カードのココロアロマ3件は縦積みとし、簡略プレビューの横並びは配色・情報量確認専用である。approved JSON release未選択、Q-006関連行status未昇格、Q-012正式release未完了はrelease readinessを妨げる別条件として維持する。通常モードの自動外部通信は0件で、CSPは`connect-src 'none'`である。F-023だけは利用者操作による同一タブ遷移を行う。JSON runtime loadingとPages deploymentは`docs/superpowers/plans/2026-07-26-csv-content-activation-pages.md`で後続対応する。

## 2. 画面一覧

| 画面ID | ルート | 名称 | 対応機能ID |
|---|---|---|---|
| S-001 | `#/start` | 開始 | F-001, F-004, F-007, F-009, F-014, F-023 |
...
| S-005 | `#/share?resultId=<UUID>` | 共有プレビュー | F-011, F-012, F-014, F-015, F-016, F-018 |
| S-006 | `#/history`／`#/sigotosocket` | 履歴／連携結果選択 | F-009, F-010, F-013, F-014, F-015, F-023 |
...
flowchart TD
    ...
    Start -->|任意連携| LinkHistory["S-006 50問だけの連携結果選択"]
    ...
    Detail -->|5因子数値を明示的に渡す| Sigotosocket["シゴトソケット"]
    LinkHistory -->|5因子数値を明示的に渡す| Sigotosocket
    Preview --> Share["S-005 共有プレビュー"]
    Detail --> Share
    History --> Preview
    History --> Detail
    History -->|50問履歴の5因子数値を明示的に渡す| Sigotosocket
...
- 初期状態で閉じた紹介見出し`自分を知る。自分と付き合う。そのためのツール。`。土や鉢のない2枚葉の芽アイコンを添え、句点後だけで折り返す
- 紹介見出しを展開した時だけ、愛猫`もなか`をイメージキャラクターとして、ツールの成り立ち、利用者にそっと付き合う目標、その時々の傾向に応じて出会えるもなかが変わることを説明する紹介文
...
- `mvp-1.3.4`等の現在アプリ版の常時表示と、診断関連版への導線
- 互換途中回答がある場合のみ「途中から再開する」。簡易プレビュー表示済みの20問進捗では「残り30問を再開する」
- 「診断結果の履歴を見る」を診断開始・再開と同じ白いメインカード内に副ボタンとして表示し、履歴0件では無効、1件以上では`#/history`へのリンクにする
- ベータ版では、回答開始ボタンより前に匿名集計を行うことを示す短い通知とS-009への導線
- ページ最下部に細い区切り線と控えめな補助文字で`© 2026 SIKUMI LAB`を表示する
...
- 「この診断について」は`start-main-panel`の外に置く。結果、履歴、比較、回答の本文へ同パネルを適用しない。
- `start-main-panel`直後へ、初期状態で閉じた`シゴトソケットとの連携方法`を置く。両アプリを単独利用できること、任意連携の手順、受け渡す情報、1件だけ・最後の結果が反映されることを説明する。
...
- 履歴管理の全削除が成功した場合は保存済み途中回答と画面内の途中回答状態を同時に破棄し、開始画面へ戻った時に再開ボタンを表示しない。取消または保存失敗時は既存状態を維持する。

## 6. S-002 回答

`renderQuestionnaireScreen`とlive controllerの回答・保存・分岐、2026-07-27承認の共通ヘッダー、明示的な中断、状態別再開、簡易プレビュー終了、新規開始時の置換確認は実装・テスト済みである。`#/answer`で設問phase、20問分岐phase、50問完答確認phaseをexact view modelとして分離し、回答、戻る、中断、破棄、互換途中回答の再開を既存state/storage APIへ接続する。保存失敗時もメモリ上の回答を維持して通知する。20問の`continueHidden`は結果を作らず21問目へ進む。`showPreview`、50問完答確認の「結果を見る」、完答確認から戻った設問画面だけに出る「回答を完了する」がanswer-freeのResultSnapshotを生成する。

### 表示

- 現在の診断区分に対する位置を示す緑のプログレスバーを置き、その直後に回答数`n / 20問`または`n / 50問`を太字で表示する。
- 通常回答と完答確認から戻った見直しのどちらも、前問操作は「前の質問」と表示する。旧「前へ」表記は使用しない。

- 設問本文
- 5件法の選択肢。値の方向を誤解しない自然言語ラベル
- 緑の進捗と、その下の設問`h1`からなる共通見出し
- 他画面と同じ通常の共通ヘッダーと、折り返さない「中断してトップへ」
- 「前の質問」「その他の操作」「回答を破棄」。破棄は中断と区別した管理操作
...
- 「回答へ戻る」で50問目へ戻り、既存回答を表示したまま見直し・変更できる。戻った後だけ、50回答が揃った各回答画面に主操作「回答を完了する」を表示する。
- 見直し中は任意の設問位置で「回答を完了する」を押すと結果を確定する。最後の設問まで進み直す必要はなく、通常回答中には同操作を表示しない。
...
20問の`showPreview`は、対応する`ProgressRecord.progressId`をそのまま`ResultSnapshot.resultId`として保存し、answer-freeの結果を`#/result?resultId=...`へ表示する。履歴から追加30問へ進めるのは、`preview20`、`showPreview`、20回答、VersionTuple一致に加え、`progress.progressId === snapshot.resultId`を満たす途中回答だけである。旧履歴の異なるIDを回答数・日時・版で推測して再リンクしない。50問完答時は新しい`resultId`を持つ`detail50`を保存し、20問結果とは別履歴にする。保存schemaのフィールドとversionは増やさない。

結果画面は称号・猫hero、称号理由、称号別`titleReflection`、名前付きレーダー、固定順5因子の行・実スコアを反映する棒・数値、因子の一回開閉、設問構成sheet、4つの固定方法sheetを表示する。因子を開くと、保存済みの全カテゴリ本文を1枚のカードへまとめて表示する。5因子とココロアロマは同時に1件だけ開け、別の因子またはアロマを開くと先の領域を閉じる。パレットはこの排他グループに入れない。5因子スコア領域へ`拡大して見る`は置かない（S-005の共有画像を拡大して見る操作は維持する）。`画像を利用できない場合も診断結果は有効です`は、称号理由の後で称号の正式タイプ注意書きの直前へ置き、両方を同じ控えめな補助文字で表示する。

### ファーストビュー

- 英字見出し`PREVIEW RESULT`と主見出し「20問簡易プレビュー」
- 主見出しは24pxとし、実施日時を直下へ小さな補助文字で表示する
- 仮称号、仮猫キャラクター
- 「本アプリ独自のプロフィール表現」
- 称号に対応する中立副題
- 称号になった理由
...
- 固定因子順の因子名、実スコアを反映する棒、数値、矢印と「詳しく見る」を含む全幅の選択行
...
共有CTAは通常フロー内にブランドアイコン付きの`今回の称号カードを見てみませんか`／`称号カードを見る`の1箇所だけ置き、「結果の根拠と見方」カードより前に表示して追従表示しない。文言は`app/js/config/ui-copy.js`へ集約する。5状態の表示・操作は次のとおりである。

| 状態 | パレット | アロマ | 共有CTA | 下部操作 | 履歴復帰 |
|---|---:|---:|---:|---|---|
| 50問・診断直後 | 表示 | 表示 | 表示 | `トップへ戻る`、`もう一度診断する` | なし |
| 50問・履歴 | 表示 | 表示 | 表示 | `履歴一覧に戻る` | ヘッダー＋下部 |
| 20問・診断直後 | 表示 | 表示 | 表示 | `50問へ進む`、`簡易プレビューで終了する` | なし |
| 20問・履歴・回答継続中 | 非表示 | 表示 | 非表示 | `50問へ進む`、`履歴一覧に戻る` | ヘッダー＋下部 |
| 20問・履歴・確定済み | 表示 | 表示 | 表示 | `履歴一覧に戻る` | ヘッダー＋下部 |

20問の回答継続中は結果の共有対象ではないため、パレットと共有CTAを表示しない。`中断してトップへ`は回答画面の操作であり、結果画面へ置かない。`簡易プレビューで終了する`は診断直後の20問結果だけで、保存済みResultSnapshotを残し対応ProgressRecordだけを削除する。

### 失敗時

- 猫画像失敗: 代替文、称号、中立副題、称号理由、5観察文、最下部の「結果の根拠と見方」への導線を維持
- Canvas失敗: HTMLの数値一覧、結果文、最下部の「結果の根拠と見方」への導線を維持
- 共有API失敗: 共有テキストを維持し、Clipboardを利用できない場合も選択可能テキストへ到達可能にする
- 履歴保存失敗: 結果を維持し、保存不可を通知

## 8. S-004 詳細結果

50問完答callerはanswer-freeのsnapshotを生成・保存し、`#/result?resultId=...`でS-004を開く。保存成功時は結果追加とProgressRecord削除を原子的に行い、保存失敗時もlive結果と文面を維持して通知し、callerは回答地図と完答ProgressRecordへの参照を破棄する。現行`result-text-v2`の通常45件は、中立副題、称号になった理由、固定順3件の`titleReflection`、続いてsection-first・`FACTOR_ORDER`順の5因子それぞれの観察文、強み、裏返り、仕事、人間関係、ストレス、問いかけ、行動ヒントで構成する。表示時は因子文だけをfactor-firstへ不変投影し、同時1因子の開閉から、該当因子の全カテゴリ見出しと本文へ1回の操作で到達できる。称号別振り返り定義が完全な3件組でない場合はsection全体を省略して42件のゼロ-reflection fallbackを表示し、部分的な振り返りは受理しない。

### 表示

- 英字見出し`DETAIL RESULT`と主見出し「50問詳細結果」
- 主見出しは24pxとし、実施日時を直下へ小さな補助文字で表示する。称号は`あなたの称号`の補助ラベルと称号名に分け、非公式表現の注意を小さな緑灰色の補足として示す
- 称号、猫、中立副題、レーダーチャート
- 称号になった理由
- 「振り返りのヒント」1件。共通補助文は設けず、各提案を「〜してみませんか。」で結ぶ。残り最大2件を「ほかのヒントを見る」で展開
- 称号の代表2因子と全5因子
  - 名前付きレーダーと、固定因子順の因子名、CSP下でも実スコアを反映する棒、数値を含む全幅の選択行
  - 初期状態では因子本文を閉じる。因子を一回開くと、「今の傾向」「活かしやすい強み」「強みの裏返り」「仕事」「人間関係」「ストレス」「振り返りと行動ヒント」の保存済み本文を1枚のカード内へ一括表示する。各カテゴリ見出しは枠線へ接触しない上・右・左の内側余白を持つ。同時に開く因子またはココロアロマは1件だけとする
  - 因子ごとの二段階開閉、汎用サマリ、三段目の「詳しく見る」、因子スコア領域の`拡大して見る`は置かない
- 因子一覧後の補助注記「因子を選ぶと、詳しい結果を確認できます。」
- 5因子の後の「結果の根拠と見方」カード先頭に置く「因子ごとの設問構成を見る」。今回の結果が20問／50問のどちらかを示し、正方向・逆方向項目数だけを表示して設問本文と回答は表示しない
...
- 共有、履歴、再診断、診断直後の「トップへ戻る」
- 50問結果だけに`この結果をシゴトソケットへ渡す`ボタンと`5つの数値だけを渡します。回答そのものは渡しません。`を表示する。診断直後と履歴詳細の両方で、利用者操作時だけ固定形式URLへ同一タブ遷移する
- 同じ枠内に、Big Fiveの結果を仕事で活かすヒントにつなげるWebアプリである旨の短い説明と、トップページへ同一タブで移動する`シゴトソケットについて見る`リンクを置く。受け渡しボタンは淡い背景、2px枠、控えめな影で副ボタンより少しだけ強調する
- 5因子以降の通常順序は`ココロパレット`、`ココロアロマ`、称号カードCTA、該当時だけの`今回の結果について`、`結果の根拠と見方`とする

猫画像、Canvasまたは共有APIが失敗しても、称号、結果文、最下部へ集約した「結果の根拠と見方」への導線、共有テキストへの経路を同じ結果画面から利用できるようにする。Clipboardも利用できない場合は選択可能テキストへ到達可能にする。F-023の対象結果が不正な場合は遷移せず通知し、詳細結果を維持する。

診断直後の50問`トップへ戻る`は保存済みResultSnapshotなら確認なしで`#/start`へ移動する。履歴保存に失敗したlive結果では、戻ると再表示できないことを確認し、取消時は同じ結果画面を維持する。履歴から開いた保存済み結果では、ヘッダーと最下部に`履歴一覧に戻る`を表示し、`トップへ戻る`と`もう一度診断する`を表示しない。履歴から開いた20問結果の継続は、対応ProgressRecordとの`progressId === resultId`を含む完全一致だけで有効にする。条件を満たす回答継続中は`50問へ進む`と`履歴一覧に戻る`だけを表示し、パレット、共有CTA、`簡易プレビューで終了する`を表示しない。条件を満たさない20問履歴は確定済みとして、パレットと単一の共有CTAを表示し、継続操作を置かない。
...
画面上では色候補を`ココロパレット`、香り候補を`ココロアロマ`として扱う。

S-003/S-004に複数パレットを表示する。

- 色候補の見出しは`ココロパレット`、副題は`〜あなたらしさから着想した色の選択〜`とする。
- パレットは常時表示し、見出し、副題、3候補、`選んだ色は、共有カードの色合いに反映されます。`の順に示す。注釈は3候補の下で右揃えとし、色見本の大きさを保ったまま上下余白を抑える。
- 標準1件と代替2件は`パレット1`、`パレット2`、`パレット3`の円形見本として表示する。選択中はブランド緑の中央`✓`と外枠、`aria-pressed`、支援技術用状態で示し、選択しても領域は閉じない。
...
香調候補は「ひと息つきたい」「気持ちを切り替えたい」「静かに取り組みたい」の固定順で、各2件、合計6件を持つ。結果画面の副題は`〜あなたらしさから着想した香りの提案〜`とする。`ココロアロマ`は閉じた状態でも3場面の代表素材画像を各1件と版付きの場面ラベルで見せるティーザーとし、素材画像は各列幅の50%相当・最大46pxに抑える。外枠、淡い面、右向き矢印で全体を押せるカードとして示す。1回開くと矢印を下向きに回転して6候補と共通注記を一括表示する。展開後の場面見出しは先頭候補との関係が自然になる上余白を持つ。場面ごとの内側アコーディオンは設けない。アロマは5因子と同じ相互排他グループに入り、パレットは開閉対象にしない。開閉状態は色・香り・診断結果の保存値へ影響させない。共有カードの既存副題`～あなたらしさから着想した香り～`は変更しない。

色選択はResultSnapshotの`selectedPaletteId`だけを更新し、保存失敗時も画面内の選択と結果全体を維持して通知する。アロマは現在の心理状態や効果を示さず、商品、配合、使用方法を案内しない。

## 10. S-005 共有プレビュー

T-007で`renderShareScreen`を実装し、`#/share?resultId=<UUID>`から対象ResultSnapshotを検証して共有モデル、PNG、共有テキストを生成する。画面状態は`data-share-view`の`card`、`details`、`zoom`で管理し、同じPNG Blobから作った同じ画像URLを再生成せずに使う。ルートを離れる時はObject URLを解放する。現行`card-template-v2`は承認済み`kokoro-wreath-v2.png`を使い、旧`kokoro-wreath-v1.png`はロールバック用に残す。

### 表示

- 英字見出し`TITLE CARD`と24pxの主見出し`称号カード`
- 実際に外部へ出る画像
- 共有テキスト全文
- 20問／50問の区別
- 中央ブランド、称号ラベルと称号、猫を隠さず自然に囲む視認可能な植物リース、透過猫、固定順5因子の棒、色候補反映、`ココロアロマ`の見出し・副題・透過素材画付き代表3件（場面名、素材例、短い印象）、共通注記、注意書き、モード、アプリ版
- 共有テキストは、ブランド、20問／50問のモード、`称号：...`、称号副題、見出しなしの称号理由、固定順5因子、`ココロアロマ`の場面／香調／`香りの素材例：...`、標準注意書き、任意URLの順で表示する。生回答、`titleReflection`、内部版ID、`この称号になった理由`見出しは含めない
- `APP_META.brand.shareUrl`が空文字ならURL行と余分な空行を表示せず、有効な空白・資格情報なしのHTTPS URLだけを注意書きの後へ空行1つを挟んで表示する。`publicOrigin`はデプロイメント情報であり共有URLの暗黙値にせず、URLは結果画面とカード画像に表示しない
- 選択配色と猫が同系色の場合も確認できる、縁取り・影を含む完成時と同一のプレビュー。`card-template-v2`は補正版の高解像度透過ラスタリースを猫より先にCanvasへ合成する。淡い暖色の円弧線を残し、上部・下部の中央を開放し、リースの見える下端を猫画像の透明画素下端付近へ合わせる。白色・ニュートラル色の円形面や背面プレートは置かない。`card-template-v1`の旧円形リースは維持する
...
利用可能な機能だけを表示する。`拡大して見る`と`結果へ戻る`は文字リンク扱いにせず、`共有内容を見る`や`カードへ戻る`と同じ副ボタンのトンマナへ揃える。`結果へ戻る`は直前の共有・保存・コピー操作から10px相当の通常間隔だけを空け、接触も過剰な分離も避ける。実行前に画像Blobとテキストの生成成功を確認する。
...
プレビュー、Web Share、PNGダウンロードは同じ生成済みPNG Blobを使う。Canvasまたは共有APIの失敗時も、コピーまたは選択可能な共有テキストへ到達できる。

Q-007確定・実装内容:

- PNGは1080×1800、3:5とする。
- 初期の`card`は320×480でも文書全体の縦スクロールを発生させず、画像、状態切替、共有可能な操作を画面内へ収める。
- `details`は画像と共有テキストの全文確認、`zoom`は画像の明示的な拡大確認としてスクロールを許可する。
- 猫画像は全体表示、中央配置、トリミングなしとする。
- 選択パレットは維持し、猫の再配色や同系色パレットの除外は行わない。
- 猫と隣接背景の分離が弱い場合は、明暗二重縁取りまたは影をカードテンプレート側で適応する。現行`card-template-v2`では白色・ニュートラル色の円形面と矩形の背景プレートを置かない。
- 現行リースは左右各1本の円弧基線から枝葉が伸びる`kokoro-wreath-v2.png`とし、猫の背面合成と下端追従を維持する。円弧の起点・最大幅・終点は承認済み座標へ固定する。
- 猫は1024×1024透過WebP、香りの素材画は短辺800px以上の透過PNG、リースは1254×1254透過PNG、ブランドは512px PNGを使う。枠・棒・文字はCanvasへ直接描画する。完成カードをSVGからラスタライズしない。低解像度SVGも制作素材に使わず、透過素材へ白い円形カバーを追加しない。
- 日本語テキストはCanvasへ描画前にフォント読込完了を待つ。
- CSPは`img-src 'self' data: blob:`で画面内生成PNGのObject URLだけを許可し、`connect-src 'none'`による通常版の自動外部送信0件を維持する。F-023の利用者操作による同一タブ遷移だけを例外とする。

2026-08-02の実ブラウザ検証では、座り猫、籠と木の実を伴う猫、手帳と札を伴う猫の3件を現行rendererから各1080×1800 PNGとして生成した。補正版リースが猫の背面にあり、見える下端が猫または最下部小物付近へ追従し、上部・下部の中央が開放されることを目視した。確認画面の横overflowとconsole warning／errorは0件だった。Blob画像のCSPと320×480の操作契約は既存回帰テストで維持する。
...
- 最後にシゴトソケットへ渡した50問詳細結果だけは、20問／50問バッジの横に小さなシゴトソケットアイコンと`連携済`を表示する。履歴から渡す操作では外部遷移前に現在のカードを再描画し、画面更新を待たず表示する。別の結果を渡すと表示は新しいカードへ移り、受信成否や連携先での削除状態は表さない
- 「結果を見る」は保存済みsnapshotを通常のS-003/S-004で開く
- 通常履歴でも50問詳細結果のカードだけに、左の`シゴトソケットへ渡す`と右の`結果を見る`からなるC案の操作列を表示する。20問簡易プレビューは`結果を見る`だけを表示する
- 空状態: 「まだ結果がありません」と診断開始導線
- `#/sigotosocket`では50問詳細結果だけを表示し、`CONNECT`／`シゴトソケットへ渡す結果を選ぶ`と説明を表示する。通常の履歴削除・比較操作は表示しない
- 連携カードは左の`シゴトソケットへ渡す`と右の`結果を見る`を16px間隔で1行に配置する。右ボタンは128px、両方16px・44px以上とし、左ボタンだけ字間をわずかに詰めて左右余白を確保する。360px以上は1行、359px以下は文字を潰さず縦積みにする
- `連携済`表示は通常履歴と連携用履歴のどちらでも同じカードへ付け、比較選択中も識別を維持する。端末内状態が不正または保存できない場合は以前の表示も消し、受け渡し操作自体は継続する。外部画面からブラウザの戻る操作でBFCache復元された場合は履歴を再描画する

### 操作

- 通常時の固定下部アクションバーから「結果を比較する」
- 明示的な`履歴削除`ボタンから既存の`データの管理`ボトムシート／モーダルを開き、日時・20問／50問で識別できる個別削除、全削除、版情報を表示
- 履歴管理の通常操作は他画面と同じ緑系の副ボタン、削除候補は淡い赤系、確認後の確定削除だけを明確な赤系とする。dialog上段は`履歴を管理する`を左、44px以上の`閉じる`を右へ同じ縦位置で配置する。`診断時の情報を見る`の展開本文は小さな補助文字とし、版名と値を読み分けられる二列表示にする
...
- 固定アクションバーの比較モードは、中央揃えの選択件数を上段、`キャンセル`と`選択した2件を比較`を折り返さない下段に置き、safe areaと本文末尾paddingを確保する。

## 12. S-007 比較

### 表示

- 共通ヘッダー右側の「履歴一覧に戻る」と、`COMPARISON`／「診断結果の比較」の共通見出し
- 比較条件を「設問数」「尺度版」「設問版」「採点版」のラベル付き項目として表示し、長い版値も項目境界を失わない
- 古い結果、今回の結果
- 前回・今回・差を同一の0〜100表示軸で示す5因子比較
- 差は94pxの固定右列へ中央揃えし、1段目に符号付き整数、2段目に状態文を表示する。`±0`と`（変化なし）`を別段にし、数字へ`tabular-nums`相当の固定桁を適用する。410px以下では因子名だけを全幅の上段とし、前回→今回と差分は左右2列に保つ。小数表示へ変更する場合は`±0.00`へ統一する
- 増減を色だけに頼らない符号と文言
- 「性格の確定的変化ではない」という注意
- 表現版が異なる場合の補足
...
| S-004 詳細結果 | 50問完答/履歴 | S-005/S-006/シゴトソケット | S-001 | 履歴から復元 | 非該当 | テキスト結果を維持。連携値不正時は遷移しない |
...
2026-07-31の結果UX追補では、CSPで無効になるインライン幅指定をネイティブ`progress`へ置き換え、0〜100の実スコアを棒へ反映した。当初は因子の1回展開で全本文を表示したが、利用者確認でスクロール負担が大きいと判明したため、因子→カテゴリ名の二段階へ戻した。カテゴリ名自体が本文の開閉操作であり、汎用サマリや三段目の「詳しく見る」は置かない。文章ごとの内部根拠ID表示は削除し、尺度・採点・限界・出典を最下部の「結果の根拠と見方」へ集約した。

同日のT-008Bと`mvp-1.0.0`確認では、履歴から開いた再開可能な20問結果を継続＋履歴一覧復帰だけにし、比較を共通0〜100表示軸・二段の差・ラベル付き版情報へ変更した。パレット／アロマとアロマ3場面は閉じて開始し、それぞれ同時1件だけ展開する。全削除後は保存領域と画面内進捗を同時に破棄し、320pxの開始画面で再開ボタンがないことを確認した。共有カードは360pxと960pxでリース、アロマ透過素材3件、副題、ブランド文字間、modeと版の非重複を実見した。320pxの開始・比較に横overflowはなく、最終console warning／errorは0件だった。

同日の追加UI確認では、360pxの開始画面で紹介本文が初期非表示、見出しタップ後だけ表示されることを確認した。20問結果ではパレット展開後に共有カードへの反映説明を表示し、代替色選択後も展開状態と選択表示を維持した。960pxの共有カードでは、素材名と香調説明をつなぐ装飾点、拡大した香調説明、最下段カードから離した共通注記、外枠と重ならない版表示を実見した。これらは過去配置の履歴証拠として扱う。現行`card-template-v2`は2026-08-02承認の補正版透過ラスタリースを猫の背面へ合成し、猫ごとのalpha下端を基準に縦位置を調整する。白色・ニュートラル色の円形面や矩形プレートは置かない。
