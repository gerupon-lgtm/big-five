...
- 共通ヘッダー直下では、`SELF CHECK`から診断開始・再開操作までを一つの白い`start-main-panel`へまとめる。`Big Fiveについて`は同パネル内の説明ブロックとし、別の大きな白カード、角丸、影を重ねない。
...
- 50問で確認できる内容
- 猫カード／共有カードプレビュー近くの標準1＋代替2の色候補と、同じ主結果グループ内で意味を分けた3場面×各2件の香調候補。各香調には1〜3件の「香りの素材例」を表示し、追加回答は要求しない
- 「あと30問続ける」
...
- 方法説明より上の「因子ごとの設問構成を見る」。20問／50問別の正方向・逆方向項目数だけを表示し、設問本文と回答は表示しない
- 「測定の土台」「スコアの計算方法」「この結果の限界」「出典・利用条件」を開くアクセシブルなボトムシートまたは同等UI
- 境界・僅差時の補足
...
- 共有テキスト全文
- 20問／50問の区別
- 猫、称号、レーダー、短文、色候補反映、代表香調候補、注意書き、版。香りの素材例は含めない
...
2026-07-30のAIリテラシー検定トンマナ整合では、commit `7bb0503`の実アプリをCodex in-app Chromium runtimeで確認した。開始画面の`scrollWidth/clientWidth`は320×800で`305/305`、360×800で`360/360`、414×896で`414/414`、960×900で`960/960`だった。ヘッダーのアイコン／アプリ名／副題は順に、320pxで`32px / 12.8px / 8.32px`、360pxで`34px / 13.44px / 8.8px`、414pxと960pxで`38px / 16.32px / 10.88px`だった。開始画面だけに白い`start-main-panel`が1個あり、角丸14px、paddingはモバイル20px・960pxで34px、入れ子の`status-card`は0個で、履歴導線と診断情報はパネル外にある。360pxの回答、履歴、結果、比較は横overflowとヘッダー内の重なりがなく、`start-main-panel`も0個だった。回答操作のnowrap、設問20px、選択肢16px・高さ56px、結果の称号→理由→振り返り→5因子の順、履歴・比較の内容を維持した。開始・回答・履歴・結果のconsole error／warningは0件で、比較操作も表示上・runtime上のerrorなく完了した。focus-visibleは既存のCSS・自動テストで確認済みだが、今回の手動キーボード再確認は行っていない。
