...
- 初期状態で閉じた紹介見出し`自分を知る。自分と付き合う。そのためのツール。`
...
共有CTAは通常フロー内に`今回の結果を残してみませんか`／`結果を共有する`の1箇所だけ置き、追従表示しない。5状態の表示・操作は次のとおりである。
...
- 称号の代表2因子と全5因子
  ...
  - 初期状態では因子本文を閉じる。因子を一回開くと、「今の傾向」「活かしやすい強み」「強みの裏返り」「仕事」「人間関係」「ストレス」「振り返りと行動ヒント」の保存済み本文を1枚のカード内へ一括表示する。同時に開く因子またはココロアロマは1件だけとする
...
- 色候補の見出しは`ココロパレット`、副題は`～あなたらしさから着想した色～`とする。
- パレットは常時表示し、見出し、副題、`選んだ色は、共有カードの色合いに反映されます。`、3候補を示す。
- 標準1件と代替2件は`パレット1`、`パレット2`、`パレット3`の円形見本として表示する。選択中は外枠と中央の`✓`、`aria-pressed`、支援技術用状態で示し、選択しても領域は閉じない。
...
香調候補は「ひと息つきたい」「気持ちを切り替えたい」「静かに取り組みたい」の固定順で、各2件、合計6件を持つ。`ココロアロマ`は閉じた状態でも3場面の代表素材画像を各1件見せるティーザーとし、1回開くと6候補と共通注記を一括表示する。場面ごとの内側アコーディオンは設けない。アロマは5因子と同じ相互排他グループに入り、パレットは開閉対象にしない。開閉状態は色・香り・診断結果の保存値へ影響させない。

色選択はResultSnapshotの`selectedPaletteId`だけを更新し、保存失敗時も画面内の選択と結果全体を維持して通知する。アロマは現在の心理状態や効果を示さず、商品、配合、使用方法を案内しない。

## 10. S-005 共有プレビュー

T-007で`renderShareScreen`を実装し、`#/share?resultId=<UUID>`から対象ResultSnapshotを検証して共有モデル、PNG、共有テキストを生成する。画面状態は`data-share-view`の`card`、`details`、`zoom`で管理し、同じPNG Blobから作った同じ画像URLを再生成せずに使う。ルートを離れる時はObject URLを解放する。2026-07-31の縦構成・代表3体と2026-08-01の360px代表1体の実ブラウザ／実画像確認は、現行6本アーチより前の配置、または置換済みの淡い円形ガイド／4本植物枝版に対する履歴証拠である。現行6本アーチ（`c56a639`）の自動geometry検証はコミット済みである。2026-08-01のTask 6では、コミット対象外の検証レポート／成果物として、異なる猫・称号・スコア・パレット・アロマを用いた代表3枚の1080×1800原寸PNGをエージェントが目視し、960px実ブラウザで共有プレビューと拡大が同一Blob URLを使うこと、console warning／error 0件、外部resource 0件を確認した。ユーザー最終確認は未完了である。
...
- 共有テキストは、ブランド、20問／50問のモード、`称号：...`、称号副題、見出しなしの称号理由、固定順5因子、`ココロアロマ`の場面／香調、標準注意書き、任意URLの順で表示する。生回答、香りの素材例、`titleReflection`、内部版ID、`この称号になった理由`見出しは含めない
...
- 選択配色と猫が同系色の場合も確認できる、縁取り・影を含む完成時と同一のプレビュー。`card-template-v2`は透明猫より先に6本のCanvas植物枝を描く開いたアーチとし、円形ストローク、白色・ニュートラル色の円、背面プレートを置かない。`card-template-v1`の旧円形リースは維持する
...
- 猫は1024×1024透過WebP、香りの素材画は短辺800px以上の透過PNG、ブランドは512px PNGを使い、枠・植物枝・棒・文字はCanvasへ直接描画する。完成カードをSVGからラスタライズしない。透過素材へ白い円形カバーを追加しない。
- 日本語テキストはCanvasへ描画前にフォント読込完了を待つ。
- CSPは`img-src 'self' data: blob:`で画面内生成PNGのObject URLだけを許可し、`connect-src 'none'`による通常版の外部送信0件を維持する。

2026-07-31の実ブラウザ検証では、1080×1800のPNG読込、1280×720と320×480の初期`card`、`details`内スクロール、`zoom`の縦横スクロール、3状態での同一画像URL、操作領域、ブラウザログ0件を確認した。正式ビジュアルの縦構成は、標準的な座り猫、動きのある猫、小物を含む横幅の広い猫の3体で、全身・小物がリース領域に収まりトリミングされないこと、アロマ3行とフッターが重ならないことを確認した。猫周囲のリース表現は後から再調整対象となったため、この確認を更新後デザインの最終承認とは扱わない。検証中に見つかったBlob画像のCSP遮断と320×480の操作重なりは、CSPと短高さレイアウトの回帰テストで解消した。
...
- 差は1段目に符号付き整数、2段目に状態文を表示する。`±0`と`（変化なし）`を別段にし、数字へ`tabular-nums`相当の固定桁を適用する。小数表示へ変更する場合は`±0.00`へ統一する
- 増減を色だけに頼らない符号と文言
- 「性格の確定的変化ではない」という注意
- 表現版が異なる場合の補足
...
同日の追加UI確認では、360pxの開始画面で紹介本文が初期非表示、見出しタップ後だけ表示されることを確認した。20問結果ではパレット展開後に共有カードへの反映説明を表示し、代替色選択後も展開状態と選択表示を維持した。960pxの共有カードでは、素材名と香調説明をつなぐ装飾点、拡大した香調説明、最下段カードから離した共通注記、外枠と重ならない版表示を実見した。これらの共有カード目視と、2026-08-01の360px代表1体実画像確認は、現行6本アーチより前の配置、または置換済みの淡い円形ガイド／4本植物枝版に対する履歴証拠である。現行`card-template-v2`は、植物だけのちょうど6本の枝を左右から伸ばし、上辺・下辺を閉じず、円形ガイド、白色・ニュートラル色の円形面、矩形プレートを置かない。猫の顔・胴体・しっぽを隠さず自然に囲む。現行6本アーチ（`c56a639`）では円弧不使用・枝数・保護領域の自動geometry検証をコミット済みである。2026-08-01のTask 6では、コミット対象外の検証レポート／成果物として、異なる猫・称号・スコア・パレット・アロマを用いた代表3枚の1080×1800原寸PNGをエージェントが目視し、960×900の実Chromeで共有プレビューと拡大が同一Blob URLを使うこと、console warning／error 0件、外部resource 0件を確認した。ユーザー最終確認は未完了である。
