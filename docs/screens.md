...
T-007で`renderShareScreen`を実装し、`#/share?resultId=<UUID>`から対象ResultSnapshotを検証して共有モデル、PNG、共有テキストを生成する。画面状態は`data-share-view`の`card`、`details`、`zoom`で管理し、同じPNG Blobから作った同じ画像URLを再生成せずに使う。ルートを離れる時はObject URLを解放する。2026-07-31に正式ビジュアルの実装、実ブラウザ確認、ユーザーの最終視覚承認を完了した。
...
- 共有テキスト全文
- 20問／50問の区別
- 中央ブランド、称号ラベルと称号、共通円形リース、透過猫、固定順5因子の棒、色候補反映、`ココロアロマ`の見出し・副題・透過素材画付き代表3件（場面名、素材例、短い印象）、共通注記、注意書き、モード、アプリ版
- 共有テキストには香りの素材例を含めない
- 選択配色と猫が同系色の場合も確認できる、縁取り・影・共通円形リース内のニュートラルな円形面を含む完成時と同一のプレビュー
...
- 猫と隣接背景の分離が弱い場合は、明暗二重縁取り、影、共通円形リース内のニュートラルな円形面をカードテンプレート側で適応する。矩形の背景プレートは置かない。
...
2026-07-31の実ブラウザ検証では、1080×1800のPNG読込、1280×720と320×480の初期`card`、`details`内スクロール、`zoom`の縦横スクロール、3状態での同一画像URL、操作領域、ブラウザログ0件を確認した。正式ビジュアルでは、標準的な座り猫、動きのある猫、小物を含む横幅の広い猫の3体で、全身・小物が共通円形リース内に収まりトリミングされないこと、アロマ3行とフッターが重ならないことを確認し、ユーザーの最終視覚承認を得た。検証中に見つかったBlob画像のCSP遮断と320×480の操作重なりは、CSPと短高さレイアウトの回帰テストで解消した。
...
同日の追加UI確認では、360pxの開始画面で紹介本文が初期非表示、見出しタップ後だけ表示されることを確認した。20問結果ではパレット展開後に共有カードへの反映説明を表示し、代替色選択後も展開状態と選択表示を維持した。960pxの共有カードでは、低濃度の円形面に隠れないリース、素材名と香調説明をつなぐ装飾点、拡大した香調説明、最下段カードから離した共通注記、外枠と重ならない版表示を実見した。360px・960pxとも横overflowはなく、console warning／errorは0件だった。
