...
T-007ではResultSnapshotから共有候補を抽出し、純粋な`createShareCardModel`でカードモデルと共有テキストを構成し、`renderShareCard`でPNG Blobを生成する。S-005は同じBlobのObject URLを`card`、`details`、`zoom`で再利用し、ルート離脱時に解放する。現行`card-template-v2`は後者の補正版透過ラスタリースを維持する。上下中央付近の膨らみを抑える形状調整は保留し、今回の結果画面・共有画面UI追補では素材とCanvas描画定数を変更しない。
...
3. `renderShareCard`が`cardTemplateVersion`を検証し、履歴互換の`card-template-v1`は旧円形リース、現行`card-template-v2`は補正版の高解像度透過ラスタリースへ振り分ける。v2では白色・ニュートラル色の円形面や背面プレートを描かない。未対応版は画像を生成せず、共有画面の選択可能テキストへフォールバックする。対応版では1080×1800のCanvasを作成し、日本語フォントの準備を待つ。
