...
2. 完答確認と50問目は保存形式を変えず、`currentIndex: 49`と50回答を維持する。「回答へ戻る」で50問目を再表示した後はpresentation controllerの画面内状態を見直し中として保持し、各回答画面に「回答を完了する」を表示する。通常回答中は表示しない。
3. 完答確認の「結果を見る」または見直し中の「回答を完了する」が`completeDetail`を呼び、50回答を結果生成callerへ渡す。`completeDetail`は50回答が揃っていれば`currentIndex`を問わず受理する。不足回答は従来どおり拒否する。ResultSnapshot保存とProgressRecord削除は同一保存境界で行う。

表示上の進捗は`(currentIndex + 1) / totalCount * 100`で算出し、`totalCount`は現在の診断区分に応じて20または50とする。これはpresentationだけの表示変更であり、回答状態・採点・保存schemaは変更しない。
...
11. 状態の読込・書込に失敗しても受け渡し遷移は維持する。書込失敗時は旧状態の削除を試み、同じ画面セッションでは未連携表示を強制して以前のカードを誤表示しない。全削除成功時も独立状態を削除する。受信成否と連携先での削除は確認しない。
12. 外部画面からブラウザの戻る操作で`pageshow.persisted === true`となった場合は、現在の履歴ルートを端末内状態から再描画し、BFCache内の古い`連携済`表示を残さない。
