...
T-007ではResultSnapshotから共有候補を抽出し、純粋な`createShareCardModel`でカードモデルと共有テキストを構成し、`renderShareCard`でPNG Blobを生成する。S-005は同じBlobのObject URLを`card`、`details`、`zoom`で再利用し、ルート離脱時に解放する。2026-07-31の縦構成・代表3体と2026-08-01の代表1体の実ブラウザ／実画像確認は、現行6本アーチより前の配置、または置換済みの淡い円形ガイド／4本植物枝版に対する履歴証拠である。現行6本アーチ（`c56a639`）は円弧不使用・枝数・保護領域の自動geometry検証だけを完了しており、Task 6の代表3体・960px・画像／ブラウザ目視とユーザー最終確認は未実施である。
