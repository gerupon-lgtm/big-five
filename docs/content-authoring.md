...
行の`status`は`draft -> reviewed -> approved`で進め、採用しない行は`rejected`にします。`reviewed`は人手承認ではありません。行statusと、Q-006の18行`result-content-approvals.csv`は別のgateです。承認者、承認日、hashを推測して記入してはいけません。
