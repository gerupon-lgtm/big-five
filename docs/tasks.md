...
| F-008 | 結果可視化 | S-003, S-004, S-007 | radar-renderer | FactorResult | T-005, T-008A | 名前付きレーダー、CSP下でも実スコアを反映する固定順5因子行・棒、同時1因子の単一開閉、1回の展開で全カテゴリ本文を直接表示、最下部の設問構成sheetと4つの固定方法sheetを実装済み |
...
| F-018 | 色・香り提案 | S-003, S-004, S-005 | presentation selector, share-card, color action aggregate | PaletteDefinition, FragranceSuggestion, FragranceMaterialDefinition, beta_color_card_action_counts | T-005, T-007, T-010 | Q-013 P-0配色・P-1香調語彙素材を承認済み。P-2〜P-6と正式runtime接続待ち |
...
- 次タスク: T-005の残ゲート。承認済みQ-006文面とQ-013 P-0配色・P-1香調語彙素材を維持し、P-2〜P-6人手承認後に候補を代替色・香りUIへ接続する。共有はT-007で進め、説明・注意事項を含むT-008Aの画面接続は実装済みである。
...
- 開始ゲート:
  ...
  - Q-013: 構造と選択規則は確定。`presentation-v2`のP-0（153パレット、用途色B、WCAG）とP-1（固定3場面、29香調、25素材、29素材関連、安全表現）は承認済み。P-2〜P-6の称号別割り当て承認とruntime接続は未完了
- 作業:
...
- live結果接続（2026-07-27完了）: `ResultSnapshot.characterAssetVersion`へ選択されたQ-012 manifest entryの`assetVersion`を、初期`selectedPaletteId`へ該当TitleProfileの`defaultPaletteId`を保存する。`VersionTuple.characterManifestVersion`の流用や仮値を禁止する。Q-013の代替色・香りUIはP-2〜P-6の人手承認まで保留する。
...
- 実装記録（2026-07-27、第1バッチ）:
  ...
  - Q-013素材例仕様（2026-07-31更新）: 香り素材を版付き独立マスタ、香調との関連を素材ID 1〜2件として管理する。通常結果は6候補すべて、共有カード画像は代表3件に素材例を表示し、共有テキストからは除外する。色はパレット単位マスタを維持する。ユーザー共有の評価コメント・象徴色・香り資料は51称号を網羅する候補検討資料としてハッシュを記録したが、直接移植・自動採用・承認済み扱いを禁止する。候補データ、監査、確認資料は作成済みだが、人手承認とruntime接続はQ-013 release gate内の後続作業である。
  ...
  - 結果UX追補（2026-07-31）: スコア棒をインラインstyle依存からネイティブ`progress`へ変更し、CSP下でも0〜100の実スコアを反映する。因子行の開閉は維持し、因子内の汎用サマリとカテゴリごとの二段目「詳しく見る」を廃止して、1回の展開で全カテゴリ見出しと本文を直接表示する。文章ごとの内部根拠ID表示は削除し、設問構成、尺度、採点、限界、出典を最下部の「結果の根拠と見方」へ集約する。
...
| Q-014結果・履歴・中断再開UI | 回答中断、preview終了、状態別再開、新規開始確認、結果hero、実スコアを反映する5因子行・棒、1回の因子展開で全カテゴリ本文を直接表示、最下部へ集約した設問構成／4方法sheet、50問トップ導線、簡潔な履歴、固定比較導線、履歴管理dialog、`result-text-v2`の称号別ヒントを実装 | T-008A完了。2026-07-31の結果UX追補のPages再確認はT-011で管理 |
...
| 色・香り実データ | `presentation-v2`の153パレットと用途色BはP-0、3場面・29香調・25素材・29素材関連はP-1で承認済み。称号別関連はdraftで、P-2〜P-6の人手承認とruntime接続は未完了 | P-2から順に称号別割り当てを人手承認 |
