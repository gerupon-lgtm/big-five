...
| F-011 | 共有プレビュー | S-005 | share-card preview | ShareCardModel | T-007, T-008C | `card-template-v2`の補正版透過ラスタリース、透明素材、画像拡大、プレビュー／Web Share／保存の同一PNG Blobを回帰確認。結果画面の因子領域へ同操作を追加しない。リース中央付近の膨らみ調整は保留 |
...
| T-007 | 共有カード・保存・コピー | F-011, F-012, F-014, F-015, F-018 | 1080×1800正式カード、透過素材、S-005、共有・保存・コピー、段階代替を実装済み。後者の補正版リース、猫ごとのalpha下端追従、読込失敗時フォールバックを実装。中央付近の膨らみ調整は保留 |
...
- 状態: DONE_WITH_CONCERNS。T-005のうちF-002/F-005/F-006/F-016に対応する`result-text-v1 initial reviewed copy`の全18 gateはapprovedで、Q-006のContent Approvalは2026-07-28に完了した。`result-text-v2`の文面、承認、domain、snapshot、結果画面接続も実装済みである。Q-013のP-0〜P-6、ES Modules runtime、結果DOMの3パレット選択・保存、ココロアロマ6候補表示、T-007共有UIと正式カード実装も完了した。後者の補正版リースを現状維持し、中央付近の膨らみ調整は保留する。approved JSON release選択とQ-012正式releaseは別ゲートとして残る。
...
- 状態: IMPLEMENTED_WITH_VISUAL_FOLLOW_UP（2026-08-02）。`#/share?resultId=<UUID>`、純粋な共有モデル、1080×1800（3:5）Canvas、同一Blobプレビュー、共有・保存・コピー、選択可能テキストへの段階フォールバックを実装済みである。後者の補正版透過ラスタリースを現行採用するが、上下中央付近の膨らみを抑える形状調整は保留する。
- 完了条件:
...
- 共有カード視覚追補（2026-08-02）: `card-template-v2`は1254×1254の補正版透過PNGを使い、淡い暖色の円弧線と左右植物で上部・下部の中央を開放する。白色・ニュートラル色の円形面や背面プレートは描かない。リース素材の読込失敗は非致命とし、`card-template-v1`の履歴再現用円形配置は変更しない。
- リース形状保留（2026-08-02）: 後者の素材を現行実装として維持する。上下位置の中央付近が外側へ膨らみ過ぎて見える点は後続の形状調整とし、今回のUI追補では`kokoro-wreath-v1.png`、Canvas配置定数、猫ごとの下端追従を変更しない。
...
- 最終自動検証（2026-08-02）: `npm.cmd test`は709件成功・失敗0件、`npm.cmd run check`は61 JavaScript files・canonical runtime version 1件、`npm.cmd run content:validate`は警告657件・エラー0件、`npm.cmd run qa:preview:build`は123 files・10,563,634 bytesで成功した。警告は未解決のcontent／character release gateに由来し、今回の共有UI・共有カード変更に対するエラーではない。
