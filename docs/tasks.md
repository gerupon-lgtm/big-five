...
- 実装記録（2026-07-27、第1バッチ）:
  ...
  - トンマナ整合検証（2026-07-30）: `npm.cmd test`は533件成功・失敗0件、`npm.cmd run check`は48 JavaScript files・canonical runtime version 1件、`npm.cmd run content:validate`は警告658件・エラー0件、`npm.cmd run qa:preview:build`は102 files・6,663,780 bytesで成功し、`git diff --check`も成功した。commit `7bb0503`のローカル実アプリでは、開始画面の`scrollWidth/clientWidth`が320×800で`305/305`、360×800で`360/360`、414×896で`414/414`、960×900で`960/960`だった。文書commit `eedbeba`での補足QAでは、360pxの20問完答分岐に進捗、見出し、簡易プレビュー、結果非表示で残り30問、回答へ戻るの3操作があり、横overflow・ヘッダー重なり・`start-main-panel`がなかった。開始360／960px、回答、分岐、履歴、結果、比較のスクリーンショットを実見し、比較は互換な50問結果2件と5因子の描画前後でconsole warning／errorが0件だった。手動focus-visibleは履歴ヘッダーの`トップ画面へ`だけをkeyboard Tabで確認し、computed outlineはsolid 3px `rgb(204, 122, 26)`だった。
  - トンマナ整合Pages確認（2026-07-30）: commit `e314cc2`をpushし、Actions run `30482247470`のbuild job `90679023727`（31秒）とdeploy job `90679164991`（21秒）が成功した。公開先の360×800開始画面はdocument `360/360`、header `336/336`、アイコン34px、アプリ名13.44px、副題8.8px、白い主パネルのpadding 20px・角丸14px・入れ子`status-card` 0個で、console warning／errorは0件、スクリーンショットも目標と一致した。他画面のPages確認は行っていない。runにはNode.js 20非推奨によりconfigure-pages／upload-artifact actionをNode 24へ強制した非失敗annotationが1件ある。
