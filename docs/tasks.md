...
| T-007 | 共有カード・保存・コピー | F-011, F-012, F-014, F-015, F-018 | 実装完了・現行v2視覚確認待ち。1080×1800正式カード、透過素材、S-005、共有・保存・コピー、段階代替を実装済み。以前の実ブラウザ確認は現行6本アーチより前の履歴証拠で、`c56a639`は自動geometry検証のみ完了。Task 6の画像／ブラウザ目視とユーザー最終確認は未実施 |
...
- 状態: DONE_WITH_CONCERNS。T-005のうちF-002/F-005/F-006/F-016に対応する`result-text-v1 initial reviewed copy`の全18 gateはapprovedで、Q-006のContent Approvalは2026-07-28に完了した。`result-text-v2`の文面、承認、domain、snapshot、結果画面接続も実装済みである。Q-013のP-0〜P-6、ES Modules runtime、結果DOMの3パレット選択・保存、ココロアロマ6候補表示、T-007共有UIと正式カード実装も完了した。現行6本アーチ（`c56a639`）は自動geometry検証のみ完了しており、Task 6の画像／ブラウザ目視とユーザー最終確認、approved JSON release選択、Q-012正式releaseは別ゲートとして残る。
...
- 状態: 実装DONE・視覚確認PENDING（現行v2）。`#/share?resultId=<UUID>`、純粋な共有モデル、1080×1800（3:5）Canvas、同一Blobプレビュー、共有・保存・コピー、選択可能テキストへの段階フォールバックは実装済みである。320×480を含む実ブラウザ確認と2026-08-01の代表1体実画像確認は、現行6本アーチより前の配置、または置換済みの淡い円形ガイド／4本植物枝版に対する履歴証拠である。現行6本アーチ（`c56a639`）は自動geometry検証だけを完了し、Task 6の画像・ブラウザ目視は未実施で、実装後最終確認は未完了である。ユーザー最終確認は別途必要である。
- 完了条件:
...
- 実装記録（2026-07-27、第1バッチ）:
  ...
  - 残る懸念: approved JSON release未選択、Q-012の正式release、現行6本アーチに対するTask 6の代表3体・960px・画像／ブラウザ目視・ユーザー最終確認である。通常runtimeの外部送信0件と`connect-src 'none'`は維持する。
...
| 共有画像の最終仕様 | 1080×1800（3:5）PNG、中央ブランド、称号、6本の植物枝だけの開いたアーチ、透過猫の全体表示、固定5因子、透過素材画付きアロマ3件の縦配置、注記・モード・アプリ版を実装済み | Q-007実装済み。現行6本アーチは自動geometry検証まで完了し、Task 6の代表3体・960px・画像／ブラウザ目視・ユーザー最終確認は未実施 |
...
| 色・香り実データ | `presentation-v2`の153パレットと用途色BはP-0、3場面・29香調・25素材・29素材関連はP-1、全51称号の選択・代替パレット・香り関連はP-2〜P-6で承認済み。ES Modules runtime、結果DOM、共有Canvasへ接続済み | T-007へ接続済み。現行6本アーチを含む共有カードのTask 6画像／ブラウザ目視とユーザー最終確認は未実施 |
