...
| F-018 | 色・香り提案 | S-003, S-004, S-005 | presentation selector, share-card, color action aggregate | PaletteDefinition, FragranceSuggestion, FragranceMaterialDefinition, beta_color_card_action_counts | T-005, T-007, T-008B, T-008C, T-010 | `ココロパレット`は常時3候補、`ココロアロマ`は閉じた3画像ティーザーから1回で6候補。因子と相互排他になるのはアロマだけで、パレットは対象外。共有カードは透過素材画の代表3件を維持し、共有テキストには香りの素材名を含めない。`card-template-v2`では6本の植物枝アーチが透過猫を隠さない |
...
| T-007 | 共有カード・保存・コピー | F-011, F-012, F-014, F-015, F-018 | 実装・エージェント視覚確認完了、ユーザー最終確認待ち。1080×1800正式カード、透過素材、S-005、共有・保存・コピー、段階代替を実装済み。`c56a639`の自動geometry検証はコミット済み。2026-08-01のTask 6では、コミット対象外の検証レポート／成果物として代表3枚の原寸画像目視と960px実ブラウザ共有確認をエージェントが完了した。ユーザー最終確認は未完了 |
...
- 状態: DONE_WITH_CONCERNS。T-005のうちF-002/F-005/F-006/F-016に対応する`result-text-v1 initial reviewed copy`の全18 gateはapprovedで、Q-006のContent Approvalは2026-07-28に完了した。`result-text-v2`の文面、承認、domain、snapshot、結果画面接続も実装済みである。Q-013のP-0〜P-6、ES Modules runtime、結果DOMの3パレット選択・保存、ココロアロマ6候補表示、T-007共有UIと正式カード実装も完了した。現行6本アーチ（`c56a639`）の自動geometry検証はコミット済みである。2026-08-01のTask 6では、コミット対象外の検証レポート／成果物として代表3枚の原寸画像目視と960px実ブラウザ共有確認をエージェントが完了した。ユーザー最終確認、approved JSON release選択、Q-012正式releaseは別ゲートとして残る。
...
- 状態: 実装DONE・視覚確認PENDING（ユーザー最終確認のみ。現行v2のエージェント確認はDONE）。`#/share?resultId=<UUID>`、純粋な共有モデル、1080×1800（3:5）Canvas、同一Blobプレビュー、共有・保存・コピー、選択可能テキストへの段階フォールバックは実装済みである。320×480を含む以前の実ブラウザ確認と2026-08-01の代表1体実画像確認は、現行6本アーチより前の配置、または置換済みの淡い円形ガイド／4本植物枝版に対する履歴証拠である。現行6本アーチ（`c56a639`）の自動geometry検証はコミット済みである。2026-08-01のTask 6では、コミット対象外の検証レポート／成果物として、異なる猫・称号・スコア・パレット・アロマを用いた代表3枚の1080×1800原寸PNGをエージェントが目視し、960px実ブラウザで共有プレビューと拡大が同一Blob URLを使うこと、console warning／error 0件、外部resource 0件を確認した。実装後最終確認は未完了であり、ユーザー最終確認が別途必要である。
- 完了条件:
...
- 共有カード視覚追補（2026-08-01）: `card-template-v2`の現行実装は、透明猫より先に描く6本のCanvas植物枝だけで、上中央と下中央が空いた大きなアーチを構成する。円形ストローク、白色・ニュートラル色の円、背面プレートは描かず、猫の顔・胴体・尻尾を避ける。猫の視認性は透過WebPを維持したまま縁取り・影で確保する。`card-template-v1`の履歴再現用円形配置は変更しない。
- 実ブラウザ検証（2026-07-31〜08-01）: 1080×1800 PNGの自然寸法、1280×720と320×480の初期`card`、`details`と`zoom`、同一画像URL再利用、文書全体の縦横スクロールなし、全操作領域、ブラウザログ0件を確認した。正式ビジュアルの縦構成は、標準的な座り猫、動きのある猫、小物を含む横幅の広い猫の3体で全身・小物の無切り抜き、当時のリース領域との位置関係、アロマ3行とフッターの非重複を確認した。2026-08-01の360px代表1体実画像確認は、その後置換された淡い円形ガイド／4本植物枝版に対する履歴証拠であり、現行6本アーチの確認へ流用しない。透過素材の寸法・alpha・透明四隅・キー色残存なしは自動検証済みで、現行6本アーチ（`c56a639`）も円弧不使用・枝数・保護領域をコミット済みの自動geometryテストで確認済みである。さらに2026-08-01のTask 6では、コミット対象外の検証レポート／成果物として、異なる猫・称号・スコア・パレット・アロマを用いた代表3枚の1080×1800原寸PNGをエージェントが目視し、960×900の実Chromeで共有プレビューと拡大が同一Blob URLを使うこと、console warning／error 0件、外部resource 0件を確認した。ユーザー最終確認は未完了である。
...
- 実装記録（2026-07-27、第1バッチ）:
  ...
  - Q-013素材例仕様（2026-07-31更新）: 香り素材を版付き独立マスタ、香調との関連を素材ID 1〜2件として管理する。通常結果は6候補すべて、共有カード画像は代表3件に素材例を表示し、共有テキストからは除外する。色はパレット単位マスタを維持する。ユーザー共有の評価コメント・象徴色・香り資料は51称号を網羅する候補検討資料としてハッシュを記録したが、直接移植・自動採用・承認済み扱いを禁止する。候補データ、監査、確認資料、P-0〜P-6人手承認、ES Modules runtime生成・接続は完了した。
  ...
  - 残る懸念: approved JSON release未選択、Q-012の正式release、現行6本アーチに対するユーザー最終確認である。Task 6の代表3枚原寸画像目視と960px実ブラウザ共有確認は、コミット対象外の検証レポート／成果物として2026-08-01にエージェントが完了した。通常runtimeの外部送信0件と`connect-src 'none'`は維持する。
...
| 共有画像の最終仕様 | 1080×1800（3:5）PNG、中央ブランド、称号、6本の植物枝だけの開いたアーチ、透過猫の全体表示、固定5因子、透過素材画付きアロマ3件の縦配置、注記・モード・アプリ版を実装済み | Q-007実装済み。現行6本アーチはコミット済み自動geometry検証に加え、2026-08-01のTask 6でエージェントによる代表3枚の原寸画像目視と960px実ブラウザ共有確認をコミット対象外の検証レポート／成果物として完了。ユーザー最終確認は未完了 |
...
| 色・香り実データ | `presentation-v2`の153パレットと用途色BはP-0、3場面・29香調・25素材・29素材関連はP-1、全51称号の選択・代替パレット・香り関連はP-2〜P-6で承認済み。ES Modules runtime、結果DOM、共有Canvasへ接続済み | T-007へ接続済み。現行6本アーチを含む共有カードは2026-08-01のTask 6でエージェントによる代表3枚の原寸画像目視と960px実ブラウザ共有確認を完了。ユーザー最終確認は未完了 |
