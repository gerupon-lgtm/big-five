...
| E-1 | 知性・想像力のhigh/middle/low語彙と根拠ID | 因子名、因子定義、公式10項目の固定は承認済み。結果文語彙は未レビュー | draft | — | high/middle/low文面バッチをContent Approval Gatesでレビューする |
...
E-1〜E-5は、ここに記した項目対応だけでは`approved`にならない。Task 4が各因子の20問・50問文面バッチをContent Approval Gatesでレビューした後に限り、その因子のgateを更新する。承認日のない`draft`へ日付を補わない。
